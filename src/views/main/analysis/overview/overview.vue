<template>
  <div class="overview">
    <bx-card title="关于">
      <div class="c-left">本项目是基于 Vue3、Vuex、VueRouter、Vite、 ElementPlus 、TypeScript、Echart5 等后台系统解决方案。</div>
    </bx-card>
    <bx-card title="技术栈">
      <bx-text-link :text-arrs="technologyStacks" />
      <bx-descriptions title="生产环境依赖" :column="2" :table-datas="dependencies" />
      <bx-descriptions title="开发环境依赖" :column="2" :table-datas="devDependencies" />
    </bx-card>
    <bx-card title="项目结构">
      <div class="c-left">
        <bx-code language="bash" :code="projectDir" />
      </div>
    </bx-card>
    <bx-card title="项目规范">
      <bx-descriptions
...
import BxDescriptions from '@/base-ui/descriptions'
import BxTextLink from '@/base-ui/text-link'
import BxCode from '@/base-ui/code'
